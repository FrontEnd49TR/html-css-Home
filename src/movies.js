...
const ulElement2 = document.querySelector(".movies-most-popular");
ulElement2.innerHTML = getMostPopularMovie();

const ulElement3 = document.querySelector(".movies-least-popular");
ulElement3.innerHTML = getLeastPopularMovie();
...
function getPosterImages() {
    ...
    return arImages.join('</li>');
}

function getMostPopularMovie() {
    arPopular[0] = moviesData.results[0];
    const popularMost = arPopular.map(movie => 
        `<li class="movie-item">
        <img class="movie-image" src="${httpPrefix}${movie.poster_path}">
        <section class="title-class">
        <label class="movie-title">${movie.original_title}</label>
    ...
    return popularMost.join('</li>');
}
function getLeastPopularMovie() {
    arPopular[0] = moviesData.results[moviesData.results.length-1];
    const popularLeast = arPopular.map(movie => 
        `<li class="movie-item">
        <img class="movie-image" src="${httpPrefix}${movie.poster_path}">
        <section class="title-class">
        <label class="movie-title">${movie.original_title}</label>
